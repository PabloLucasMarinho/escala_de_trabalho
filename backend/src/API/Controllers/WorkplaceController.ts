...
import GetAllWorkplaceUseCase from "../../application/useCases/workplace/GetAll/GetAllWokplaceUseCase.js";
...
export default class WorkplaceController {
  constructor(
    ...
    @inject(GetAllWorkplaceUseCase) private getAllWorkplaceUseCase: GetAllWorkplaceUseCase,
  ...
  async Register(req: InputData<RequestWorkplaceJson>, res: Response<string>): Promise<void> {
    await this.registerWorkplaceUseCase.Execute(req);

    res.status(201).json("Cadastro realizado com sucesso.");
  }

  async GetAll(_req: InputData<null>, res: Response<ResponseWorkplaceJson[] | null>): Promise<void> {
    const response = await this.getAllWorkplaceUseCase.Execute();
